...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = mysqli_prepare($conn, "INSERT INTO posts (title, slug, content) VALUES (?, ?, ?)");
...
<body class="py-8">
  <div class="max-w-4xl mx-auto px-4">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-dark">Create New Post</h1>
        <p class="text-gray mt-2">Write and publish a new blog post</p>
      </div>
      ...
      <form method="post" class="p-6 md:p-8">
        ...
        <div class="mb-6 slug-preview">
          ...
          <div class="px-4 py-2 bg-gray-50 border border-grayLight rounded-lg text-gray">
            <span id="slug-preview-text">your-title-will-appear-here</span>
          </div>
        ...
        <div class="mb-6">
          ...
          <textarea 
            ...
            required
            class="form-textarea w-full px-4 py-3 border border-grayLight rounded-lg focus:border-primary transition-colors"
          ></textarea>
        </div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end space-y-4 sm:space-y-0 sm:space-x-4">
          ...
          >
            <i class="fas fa-plus-circle mr-2"></i> Publish Post
          </button>
        </div>
      </form>
    </div>
    ...
    <div class="mt-8 bg-blue-50 rounded-xl p-6 border border-blue-100">
      <h3 class="font-semibold text-dark mb-3 flex items-center">
        <i class="fas fa-lightbulb text-blue-500 mr-2"></i> Writing Tips
      </h3>
      <ul class="text-sm text-dark space-y-2">
        <li class="flex items-start">
          <i class="fas fa-check text-blue-500 mt-1 mr-2 text-xs"></i>
          <span>Write a compelling title that captures attention</span>
        </li>
        <li class="flex items-start">
          <i class="fas fa-check text-blue-500 mt-1 mr-2 text-xs"></i>
          <span>Break up your content with subheadings and paragraphs</span>
        </li>
        <li class="flex items-start">
          <i class="fas fa-check text-blue-500 mt-1 mr-2 text-xs"></i>
